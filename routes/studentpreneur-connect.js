...
router.post('/g_f',async (req,res)=>{
    console.log("postg_f")
    console.log(bootcamp.find({BOOTCAMP_LOCATION:req.body.BOOTCAMP_LOCATION}))
    await bootcamp.find({BOOTCAMP_LOCATION:req.body.BOOTCAMP_LOCATION})
    .then(bootcamps=>res.json(bootcamps))
    .catch(err=>res.status(400).json('error'+err));
})
...
router.post('/g_f1',async (req,res)=>{
    console.log(Student.find({clgName:req.body.clgName}))
    await Student.find({clgName:req.body.clgName})
    .then(Students=>res.json(Students))
    .catch(err=>res.status(400).json('error'+err));
})
/*
